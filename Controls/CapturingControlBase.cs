...
public abstract class CapturingControlBase : Control
{
    ...
    protected bool IsCaptureActive => SharedPointerCapture.IsOwnedBy(this);

    protected bool BeginCapture(IPointer pointer, Point position)
        => SharedPointerCapture.TryBegin(this, pointer, position);

    protected void EndCapture(IPointer? pointer = null)
        => SharedPointerCapture.End(this, pointer);

    protected bool TryConsumeCaptureWarpMove()
        => SharedPointerCapture.TryConsumeWarpSuppressedMove(this);

    protected Vector GetCaptureDelta(Point position)
        => SharedPointerCapture.GetDelta(this, position);
...
public readonly struct ControlCaptureApi
{
    private readonly Control owner;

    public ControlCaptureApi(Control owner) => this.owner = owner;

    public bool IsActive => SharedPointerCapture.IsOwnedBy(owner);

    public bool Begin(IPointer pointer, Point position)
        => SharedPointerCapture.TryBegin(owner, pointer, position);

    public void End(IPointer? pointer = null)
        => SharedPointerCapture.End(owner, pointer);

    public bool TryConsumeWarpMove()
        => SharedPointerCapture.TryConsumeWarpSuppressedMove(owner);

    public Vector GetDelta(Point position)
        => SharedPointerCapture.GetDelta(owner, position);

    public bool TryWrap(Point position)
        => SharedPointerCapture.TryWrapAround(owner, position);
}
